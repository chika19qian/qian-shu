...
Fonts are served locally from `portfolio-assets/fonts/`, with the two first-screen fonts preloaded. `font-display: optional` keeps a slow font download from replacing visible text mid-page; a scaled fallback keeps the handwritten headings compact. Font licenses are included beside the files.
